...
implementation{
    ...
    */
    ...
    uint32_t gcd(uint32_t a, uint32_t h){
      uint32_t temp;
      while(TRUE){
        temp = a%h;
        if(temp==0){
    ...
    int32_t ExtEuclid(int32_t a, int32_t b){
      int32_t x = 0;
      int32_t y = 1;
      int32_t u = 1;
      int32_t v = 0;
    ...
    int32_t ExtendEuclid(int32_t a, int32_t b, int32_t* x, int32_t* y){

    }

    int32_t ExtendedEuclidean(int32_t a, int32_t b, int32_t* x, int32_t* y){
      int32_t xTemp, yTemp;
      int32_t d;
    ...
    bool bboverflow(uint32_t a, uint32_t b){
      size_t aBits, bBits;
      aBits = highestOne(a);
      bBits = highestOne(b);
      return(aBits+bBits>=32);
      //return(aBits+bBits>=58); //<- conservative tests to catch all overflow (will catch non overflow)
    }

    uint32_t rsa_bbcalc(uint32_t b1, uint32_t b2, uint32_t m){
      b1 = b1 % m;
      b2 = b2 % m;
      if (!bboverflow(b1,b2)){
      return(b1*b2 % m);
    ...
    uint32_t rsa_modExp(uint32_t b, uint32_t e, uint32_t m){
      uint32_t tempB;
      if(b<0 || e<0 || m<=0){
        exit(1);
      }
    ...
    int32_t inverse(int32_t a, int32_t n){
      int32_t t, r, newt, newr, quotient, temp;
      t = 0;
      r = n;
      newt = 1;
      newr = a;
  ...
  command uint32_t RSAinterface.encrypt_shared_key(uint32_t key, int32_t exponent, int32_t modulus){
    key = rsa_modExp(key, exponent, modulus);
    return key;
  }

  command uint32_t RSAinterface.decrypt_shared_key(uint32_t key){
    key = rsa_modExp(key, privKey.exponent, privKey.modulus);
    return key;
  }
}
