...
implementation{
  ...
  command uint64_t RSAinterface.gen_shared_key(){
    uint64_t num;
  ...
  command uint64_t RSAinterface.encrypt_shared_key(uint64_t key, int64_t exponent, int64_t modulus){
    key = rsa_modExp(key, exponent, modulus);
    return key;
  }

  command uint64_t RSAinterface.decrypt_shared_key(uint64_t key){
