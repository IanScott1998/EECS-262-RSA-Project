...
interface RSAinterface{
    ...
    command uint64_t gen_shared_key();

    command uint64_t encrypt_shared_key(uint64_t key, int64_t exponent, int64_t modulus);

    command uint64_t decrypt_shared_key(uint64_t key);
}
